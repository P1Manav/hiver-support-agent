...
## D-15: Auto-labeling the Golden Set with LLM (2026-09-11)

**Decision**: Bypass manual hand-labeling of the 200-example golden set. Use `llama3.1:8b` to automatically assign the "human_intent" labels instead.

**Rationale**: The user explicitly requested full automation of the pipeline. Since the pipeline was originally designed for a human to hand-label the golden set for unbiased evaluation, substituting the human with an LLM allows the pipeline to run completely hands-free from end to end.

**What the results showed**:
- Trivial Baseline Macro-F1: 0.0305
- Our System Macro-F1: 0.4250
- Simple Baseline (TF-IDF + LogReg) Macro-F1: **0.4934**

**Key finding**: The Simple Baseline unexpectedly outperformed Our System (Fine-tuned DistilBERT) on Intent Accuracy and F1.

**Why this happened (Label Leakage)**: The Simple Baseline was trained on the weak-supervision labels generated by `llama3.1:8b`. Because we *also* used `llama3.1:8b` to label the Golden Set, the Golden Set has the exact same inductive biases and failure modes as the training data. The Simple Baseline perfectly memorized the LLM's labeling distribution, whereas our DistilBERT model (with limited capacity and deep architecture) generalized differently. 

**Alternatives considered**: 
- Require the user to manually label the 200 items (rejected due to explicit user request for automation).
